...
- (void)startPrefetchingAtIndex:(NSUInteger)index
{
    ...
    [self.manager downloadWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished)
    {
        ...
        if (image)
        {
            WHLog(@"Prefetched %zd out of %zd", self.finishedCount, self.prefetchURLs.count);
        }
        else
        {
            WHLog(@"Prefetched %zd out of %zd (Failed)", self.finishedCount, [self.prefetchURLs count]);
...
- (void)reportStatus
{
    ...
    WHLog(@"Finished prefetching (%zd successful, %zd skipped, timeElasped %.2f)", total - self.skippedCount, self.skippedCount, CFAbsoluteTimeGetCurrent() - self.startedTime);
}
