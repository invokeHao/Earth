...
//@property (nonatomic, strong) CMSFunctionToolView *toolView;
...
- (instancetype)initWithPhotoItems:(NSArray<KSPhotoItem *> *)photoItems selectedIndex:(NSUInteger)selectedIndex {
    ...
    if (self) {
        ...
        _pageindicatorStyle = KSPhotoBrowserPageIndicatorStyleText;
...
- (void)viewDidLoad {
    ...
    if (_pageindicatorStyle == KSPhotoBrowserPageIndicatorStyleDot) {
        ...
        _pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height-40-HOME_INDICATOR_HEIGHT, 60, 20)];
        _pageLabel.center = CGPointMake(VERTICAL_SCREEN_WIDTH/2, self.view.bounds.size.height-30-HOME_INDICATOR_HEIGHT);
    ...
    [_downLoadBtn setFrame:CGRectMake(VERTICAL_SCREEN_WIDTH - 16 - 28, CGRectGetMaxY(_pageLabel.frame)-23, 28, 28)];
    [_downLoadBtn setImage:[UIImage imageNamed:@"ico_download"] forState:UIControlStateNormal];
    [_downLoadBtn addTarget:self action:@selector(preformToDownLoad) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_downLoadBtn];
    
    _closeButton = UIButton.button.WH_setImage_forState(WHImageNamed(@"ico_close"),UIControlStateNormal).WH_frame(CGRectMake(10, 10, 40, 40));
    @weakify(self)
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (sourceRect.origin.x<1&&sourceRect.origin.y<1) {
        sourceRect = CGRectMake(VERTICAL_SCREEN_WIDTH/2, VERTICAL_SCREEN_HEIGHT/2, 2, 2);
    }
    ...
    if (endRect.size.height > (VERTICAL_SCREEN_HEIGHT * 1.5)) {
        CGRect imageRect = CGRectMake(10, sourceRect.origin.y, VERTICAL_SCREEN_WIDTH, endRect.size.height - sourceRect.origin.y);
        photoView.imageView.frame = imageRect;
    }
    if (_bounces) {
...
- (void)dismissAnimated:(BOOL)animated {
    ...
    if (self.dismissBlock) {
        self.dismissBlock(_currentPage);
    }
    KSPhotoItem *item = [_photoItems objectAtIndex:_currentPage];
    if (animated) {
        [UIView animateWithDuration:kAnimationDuration animations:^{
...
- (void)showDismissalAnimation {
    ...
    if (_bounces) {
        ...
        if (photoView.imageView.frame.size.height > (VERTICAL_SCREEN_HEIGHT * 1.5 )) {
            CGRect imageRect = CGRectMake(photoView.imageView.x , sourceRect.origin.y - 20 , photoView.imageView.width,sourceRect.size.height + 20);
            photoView.imageView.frame = imageRect;
            photoView.imageView.image = item.thumbImage;
        }
...
#pragma mark- 下载图片
- (void)preformToDownLoad {
    KSPhotoView *photoView = [self photoViewForPage:_currentPage];
    NSString * imageUrl = [photoView.item.imageUrl absoluteString];
    if (photoView.item.finished) {
        @weakify(self);
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            @strongify(self);
            if (status==PHAuthorizationStatusAuthorized) {
                UIImage * image = [_imageManager imageFromMemoryForURL:photoView.item.imageUrl];
                dispatch_async(dispatch_get_main_queue(), ^{
                    if ([imageUrl containsString:@"gif"]) {
                        [self loadGifData:image];
                    }else{
                        [self loadImageFinished:image];
                    }
                });
            }else{
                [self openThePrivacyWithTitle:@"没有相册权限，是否去设置权限"];
            }
        }];
    }else{
        [self.view showString:@"图片尚未加载完毕"];
    }
}

-(void)openThePrivacyWithTitle:(NSString*)title{
    [[WHAlertTool shareInstance]showAlterViewWithMessage:title andDoneBlock:^(UIAlertAction * _Nonnull action) {
        NSURL *settingUrl = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
        if ([[UIApplication sharedApplication] canOpenURL:settingUrl]) {
            [[UIApplication sharedApplication] openURL:settingUrl options:@{} completionHandler:NULL];
        }
    }];
}
...
- (void)loadImageFinished:(UIImage *)image {
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if(error){
                [self.view showString:@"图片保存失败"];
            }else{
                [self.view showString:@"图片保存成功"];
            }
        });
    }];
}

- (void)loadGifData:(UIImage*)image {
    YYImage * Yimage = (YYImage *)image;
    NSData * gifData = Yimage.animatedImageData;
    [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
