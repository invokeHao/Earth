...
@implementation MFYVideoManager
    ...
    for (int i = 0; i < assets.count; i++) {
        ...
        dispatch_async(queue, ^{
            ...
            if (original) {
                ...
                CGFloat scale = 2;
                CGFloat width = MIN(VERTICAL_SCREEN_WIDTH, kMaxImageWidth);
                CGSize size = CGSizeMake(width*scale, width*scale*asset.pixelHeight/asset.pixelWidth);
                @weakify(self);
...
{
//    float duration = round(asset.duration.value) / asset.duration.timescale;
...
{
    [self export:asset range:range presetName:AVAssetExportPresetPassthrough renderSize:CGSizeZero watermarkImage:nil imageSize:CGSizeZero effectImage:nil birthRate:0 velocity:0 complete:^(NSString *exportFilePath, NSError *error) {
        if (!error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if (complete) complete(YES, nil, exportFilePath);
            });
        } else {
            dispatch_async(dispatch_get_main_queue(), ^{
...
+ (void)exportVideoForAsset:(PHAsset *)asset complete:(void (^)(NSString *, NSError *))complete
{
    [self exportVideoForAsset:asset presetName:AVAssetExportPresetMediumQuality complete:complete];
}

+ (void)exportVideoForAsset:(PHAsset *)asset presetName:(NSString *)presetName complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset presetName:presetName renderSize:CGSizeZero watermarkImage:nil imageSize:CGSizeZero effectImage:nil birthRate:0 velocity:0 complete:complete];
}

+ (void)exportVideoForAsset:(PHAsset *)asset renderSize:(CGSize)renderSize complete:(void (^)(NSString *, NSError *))complete
{
    [self exportVideoForAsset:asset renderSize:renderSize watermarkImage:nil imageSize:CGSizeZero complete:complete];
}

#pragma mark - 视频加水印及粒子效果
+ (void)exportVideoForAsset:(PHAsset *)asset renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset presetName:AVAssetExportPresetHighestQuality renderSize:renderSize watermarkImage:watermarkImage imageSize:imageSize effectImage:nil birthRate:0 velocity:0 complete:complete];
}

+ (void)exportVideoForAsset:(PHAsset *)asset presetName:(NSString *)presetName watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset presetName:presetName renderSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) watermarkImage:watermarkImage imageSize:imageSize effectImage:nil birthRate:0 velocity:0 complete:complete];
}

+ (void)exportVideoForAsset:(PHAsset *)asset presetName:(NSString *)presetName effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString *, NSError *))complete
{
    [self export:asset presetName:presetName renderSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) watermarkImage:nil imageSize:CGSizeZero effectImage:effectImage birthRate:birthRate velocity:velocity complete:complete];
}

//privite
+ (void)export:(PHAsset *)asset presetName:(NSString *)presetName renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString *, NSError *))complete
{
    if (asset.mediaType != PHAssetMediaTypeVideo) {
    ...
    options.networkAccessAllowed = YES;
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset * _Nullable asset, AVAudioMix * _Nullable audioMix, NSDictionary * _Nullable info) {
        [self export:asset range:CMTimeRangeMake(kCMTimeZero, kCMTimePositiveInfinity) presetName:presetName renderSize:renderSize watermarkImage:watermarkImage imageSize:imageSize effectImage:effectImage birthRate:birthRate velocity:velocity complete:^(NSString *exportFilePath, NSError *error) {
            dispatch_async(dispatch_get_main_queue(), ^{
...
+ (void)export:(AVAsset *)asset range:(CMTimeRange)range presetName:(NSString *)presetName renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity complete:(void (^)(NSString *exportFilePath, NSError *error))complete
{
    ...
    if (!CGSizeEqualToSize(renderSize, CGSizeZero)) {
        AVMutableVideoComposition *com = [self getVideoComposition:asset renderSize:renderSize watermarkImage:watermarkImage imageSize:imageSize effectImage:effectImage birthRate:birthRate velocity:velocity];
        if (!com) {
            if (complete) {
...
+ (AVMutableVideoComposition *)getVideoComposition:(AVAsset *)asset renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity
{
    ...
    if (watermarkImage || effectImage) {
        [self addWatermark:videoComposition renderSize:cropSize watermarkImage:watermarkImage imageSize:imageSize effectImage:effectImage birthRate:birthRate velocity:velocity];
    }
...
+ (void)addWatermark:(AVMutableVideoComposition *)videoCom renderSize:(CGSize)renderSize watermarkImage:(UIImage *)watermarkImage imageSize:(CGSize)imageSize effectImage:(UIImage *)effectImage birthRate:(NSInteger)birthRate velocity:(CGFloat)velocity
{
